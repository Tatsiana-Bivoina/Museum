// video slider
const mySwiper = new Swiper('.swiper.video-slider', {
  slidesPerView: 3,
  speed: 800,
  spaceBetween: 42,
...
const btnNext = document.querySelector('.swiper.video-slider .swiper-button-next');
const btnPrev = document.querySelector('.swiper.video-slider .swiper-button-prev');
const paginationBullets = document.querySelectorAll('.swiper.video-slider .swiper-pagination');
const slides = document.querySelectorAll('.swiper.video-slider .swiper-slide');
...
function changeMainVideo() {
  let slideIndex = mySwiper.activeIndex - 3;
  ...
  let videos = ['./assets/video/video3.mp4', './assets/video/video1.mp4', './assets/video/video2.mp4', './assets/video/video4.mp4', './assets/video/video0.mp4']
  let posters = ['./assets/video/poster3.jpg', './assets/video/poster1.jpg', './assets/video/poster2.jpg', './assets/video/poster4.jpg', './assets/video/poster0.jpg']
  mainVideo.setAttribute('src', videos[slideIndex]);
  mainVideo.setAttribute('poster', posters[slideIndex]);
...
function togglePlayVideo() {
  if(mainVideo.paused) {
    ...
    btnPlay.innerHTML = '<svg width="21" height="30" viewBox="0 0 21 30" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 29.0471V0.944802C8 0.421605 7.55026 0 6.98347 0H1.01653C0.449742 0 0 0.421605 0 0.944802V29.0471C0 29.5703 0.449742 30 1.01653 30H6.98347C7.55026 30 8 29.5784 8 29.0471Z" fill="#B3B3B3"/><path d="M19.9835 0H14.0165C13.4584 0 13 0.421605 13 0.944802V29.0471C13 29.5703 13.4497 30 14.0165 30H19.9835C20.5416 30 21 29.5784 21 29.0471V0.944802C21 0.421605 20.5503 0 19.9835 0Z" fill="#B3B3B3"/></svg>';
  } else {
    mainVideo.pause();
    videoPlay.classList.remove('hidden');
    btnPlay.innerHTML = '<svg width="23" height="30" viewBox="0 0 23 30" fill="B3B3B3"><path d="M22.35 14.75L0 0C0 22.23 0 11.32 0 29.49L22.35 14.75Z"/></svg>';
  }
  mainVideo.volume = soundRange.value / 100;
}

function toggleFullScreen() {
  if (!document.fullscreenElement) {
    customPlayer.requestFullscreen();
    fullscreen.innerHTML = '<svg class="btn-fullscreen" width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M28.2554 7.74495H35.9764V12.8119H23.1874V0.0229492H28.2544V7.74395L28.2554 7.74495ZM23.1884 35.976V23.1869H35.9774V28.2539H28.2564V35.9749H23.1894L23.1884 35.976ZM7.74544 7.74495V0.0239492H12.8124V12.8129H0.0234375V7.74595H7.74444L7.74544 7.74495ZM0.0244375 28.2549V23.1879H12.8134V35.977H7.74644V28.2559H0.0254375L0.0244375 28.2549Z" fill="#B3B3B3"/></svg>';
    ...
    if (document.exitFullscreen) {
      ...
      fullscreen.innerHTML = '<svg class="btn-fullscreen" width="36" height="36" viewBox="0 0 36 36" fill="none"><path d="M31.48 31.6299H23.05V35.8599H35.7001V23.1499H31.48V31.6299Z"/><path d="M4.22 23.1499H0V35.8599H12.65V31.6299H4.22V23.1499Z"/><path d="M0 12.71H4.22V4.24H12.65V0H0V12.71Z"/><path d="M23.05 0V4.24H31.48V12.71H35.7001V0H23.05Z"/></svg>';
...
function keyHandler(e){
  ...
  if(e.shiftKey && e.code == 'Comma') {
    ...
    videoSpeedBlock.innerHTML = `x${mainVideo.playbackRate}`;
    setOpacitySpeedBlock();
  }
...
videoPlay.addEventListener('click', togglePlayVideo);
btnPlay.addEventListener('click', togglePlayVideo);
mainVideo.addEventListener('click', togglePlayVideo);
mainVideo.addEventListener('timeupdate', ()=> {
  let value = (100 / mainVideo.duration) * mainVideo.currentTime;
...
progress.addEventListener("input", () => {
  ...
  progress.style.background = `linear-gradient(to right, #710707 0%, #710707 ${value}%, #C4C4C4 ${value}%, #C4C4C4 100%)`
  let time = mainVideo.duration * (progress.value / 100);
